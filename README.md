...
> 🆕 更新说明：下载歌曲已升级为支持主流平台（网易云、酷我、Bilibili等）的服务器端流式附件下载。本方案彻底解决了安卓手机、平板等移动设备浏览器/WebView 中点击下载报错或无反应的问题，完美绕过了跨域（CORS）与混合内容（Mixed Content）限制，并且在启用了口令密码登录的环境下同样保持正常下载，文件名自动命名为 `歌名 - 歌手.扩展名`。
