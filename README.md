# 🎶 Solara（光域）
>更新：
> 1，🆕 更新说明：下载歌曲已改为直接拉取音频文件并触发浏览器保存，避免先弹出原生播放器面板；下载文件名保持为 歌名 - 歌手.扩展名。
> 2，🆕 更新说明：下载歌曲已升级为支持主流平台（网易云、酷我、Bilibili等）的服务器端流式附件下载。本方案彻底解决了安卓手机、平板等移动设备浏览器/WebView 中点击下载报错或无反应的问题，完美绕过了跨域（CORS）与混合内容（Mixed Content）限制，并且在启用了口令密码登录的环境下同样保持正常下载，文件名自动命名为 `歌名 - 歌手.扩展名`。
...
- **支持全自动深浅色模式（自适应主题）**：全面引入移动端动态语义化 CSS 变量，移除了所有移动端硬编码的绝对色彩（如纯黑背景或强制白字）。搜索界面、滑出面板、列表项、音量/进度滑块轨道、下拉选项菜单与滚动条等在浅色模式下呈现亮丽通透的亮白雾面玻璃质感，在深色模式下呈现高级幽邃的暗夜微光质感，实现完全的主题自适应与高对比度易读性，彻底告别“灰蒙蒙”；
- 重新设计的移动端音乐搜索面板，采用卡片式双行 Grid 布局解决文字截断，且色彩、边框与首页主题完全统一；
- 播放/收藏列表升级为提亮版双行卡片布局，将歌名与歌手分层排版，大幅提升深色背景下的对比度与视觉美感；
- 播放/收藏列表右侧的操作按钮（收藏、下载、添加、移除）重塑为带微光描边的触觉感圆形底座，交互时伴随醒目的功能状态高亮；
- 提亮了整个底部滑出面板的背景偏光、非激活 Tab 标签与头部控制按钮，并在超窄屏幕下自动调整布局以防止组件重叠；
