...
> 3，🆕 更新说明：移动端深浅色主题切换现已改为**完全手动模式**（不再跟随系统偏好）。默认使用浅色主题，可通过工具栏的 ☀️/🌙 按钮随时手动切换，选择会被持久化到 `localStorage`。
> 4，🆕 更新说明：彻底修复了移动端播放列表页和搜索页被一层暗色遮罩覆盖导致文字"灰蒙蒙"的问题。根因是面板背景为半透明 + `backdrop-filter` 产生的层叠上下文异常导致暗色 scrim 渗透。现已将面板背景改为全不透明，并将 scrim 元素移出 `.container`，从根本上消除了遮罩渗透。
> 5，🆕 更新说明：优化搜索结果排版，缩小了歌名与歌手的字号，使列表展示更紧凑精简。
> 6，🆕 更新说明：完美自适应 iPadOS 及各类宽屏触屏平板设备。通过升级移动端与平板触控检测机制（全面兼容 iPad 仿真 macOS 的 UA），解决了平板设备无法拉起移动端专属**黑胶唱盘旋转动画**和**歌词面板**的 Bug。唱片和歌词面板直径由 `--mobile-platter-size` 响应式 CSS 变量控制，在平板上等比例扩大至 `420px` / `460px`，极致饱满舒展；同时重构了容器的高度逻辑，彻底修复了 Flexbox 弹性高度溢出导致底部控制栏贴死屏幕边缘的缺陷，实现精致的上下对称悬浮留白。
...
- **手动深浅色主题切换**：工具栏内置 ☀️/🌙 切换按钮，点击即可在亮色与暗色主题间自由切换（不再跟随系统偏好），选择会自动持久化。全面引入移动端动态语义化 CSS 变量，搜索界面、滑出面板、列表项、音量/进度滑块轨道、下拉选项菜单与滚动条等在浅色模式下呈现亮丽通透的质感，在深色模式下呈现高级幽邃的暗夜微光质感；
- **面板全不透明渲染**：播放列表面板与搜索面板采用全不透明背景，彻底消除因半透明背景与暗色遮罩层叠导致的文字"灰蒙蒙"渗透问题，确保所有文字清晰锐利；
- 重新设计的移动端音乐搜索面板，采用卡片式双行 Grid 布局解决文字截断，且色彩、边框与首页主题完全统一；
- **平板设备大屏完美自适应**：精准适配 iPadOS 触屏平板，还原并显示专属的**黑胶唱盘旋转与等比例大歌词面板**。黑胶唱片与歌词面板的直径采用 `--mobile-platter-size` 响应式 CSS 变量控制，在平板大屏上等比例自适应扩大至 `420px`（屏幕宽 >=768px）和 `460px`（屏幕宽 >=1024px），带来尊贵饱满的宽屏视觉效果；
- **完美悬浮间隙**：主卡片容器 `.container` 的高度限制从弹性 `min-height` 重构为严格的 `height` 与 `max-height` 双锁限制，彻底告别 Flexbox 布局高度溢出导致底部控制栏贴死屏幕边缘的 bug，实现完美的上下对称悬浮留白；
- 搜索结果文字精简紧凑，歌名与歌手字号经优化调小，小屏幕下可展示更多内容而无需频繁滑动；
